...
class client_updater extends client {
    ...
    function backup_database() {
		...
		$cmd = "mysqldump -h{$cfg_clean->dbhost} -u{$cfg_clean->dbuser} -p{$cfg_clean->dbpass} {$cfg_clean->dbname}| gzip -9 > $target/{$cfg_clean->dbname}_" . date("Ymd") . ".sql.gz";
		self::log($cmd);
		shell_exec($cmd);
	}
    ...
    function get_or_create_context($context_level, $instance_id) {
		...
		$get_or_create_context	= self::$target_folder . "{$this->csv_line->domain}/public_html/get_or_create_context.php";
